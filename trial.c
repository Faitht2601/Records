...
struct Student {
    ...
    int number;
    int marks;
    int teacherIndex; // Index of the teacher who uploaded the grade
};
...
void uploadGrades(struct Student *students, int numStudents, int teacherIndex) {
    for (int i = 0; i < numStudents; i++) {
        printf("Enter grade for %s (Student %d): ", students[i].name, students[i].number);
        scanf("%d", &students[i].marks);
        students[i].teacherIndex = teacherIndex; // Set teacherIndex when uploading grades
        printf("%s %d %d %d\n", students[i].name, students[i].number, students[i].marks, students[i].teacherIndex);
    }
}

// Function to edit grades
void editGrades(struct Student* students, int numStudents, int loggedInTeacher) {
    ...
    int found = 0;
    for (int i = 0; i < numStudents; i++) {
        if (students[i].number == id && students[i].teacherIndex == loggedInTeacher) {
    ...
    if (found == 0) {
        printf("\nNo student found with ID %d or you don't have permission to edit.\n", id);
    }
}
...
void calculateStatistics(struct Student* students, int numStudents) {
    ...
    for (int i = 0; i < numStudents ; i++) {
        ...
        printf("\nMaximum mark: %d (Students with maximum marks: %s)\n", max, studentsWithMax);
    }
            ...
            if (loggedInTeacher != -1) { // if loggedInTeacher return -1 means fail
                ...
                while (1) {
                    ...
                    switch (teacherChoice) {
                        case 1:
                            uploadGrades(students, numStudents, loggedInTeacher);
                            break;

                        case 2:
                            editGrades(students, numStudents, loggedInTeacher);
                            break;
